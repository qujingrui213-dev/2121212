...
async function handleQueenieAI(request, env) {
  ...
  let body = {};
  try { body = await request.json(); } catch (_) { return json({ answer: '请输入一个问题。' }, 400); }
  const message = String(body.message || '').trim().slice(0, 500);
  if (!message) return json({ answer: '请输入一个问题。' }, 400);

  const key = env && env.QWEN_API_KEY;
  const base = env && env.QWEN_BASE_URL;
  if (!key || !base) return json({ answer: fallback(message), mode: 'fallback' });
...
${KNOWLEDGE}`;
  ...
  const endpoint = String(base).replace(/\/$/, '') + '/chat/completions';
  try {
    const res = await fetch(endpoint, {
      method: 'POST',
      headers: { 'content-type': 'application/json', authorization: `Bearer ${key}` },
      body: JSON.stringify({ model: (env && env.QWEN_MODEL) || 'qwen3.8-flash', messages: [{ role:'system', content:system }, ...history, { role:'user', content:message }], temperature: 0.25, max_tokens: 320 }),
    });
    ...
    return json({ answer: fallback(message), mode: 'fallback' });
  }
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/queenie-ai') return handleQueenieAI(request, env);
